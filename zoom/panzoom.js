...
function createPanZoom(domElement, options) {
  ...
  function onTouch(e) {
    // Deja que el anular el comportamiento táctil
    beforeTouch(e);
    if (e.touches.length === 1) {
      return handleSingleFingerTouch(e, e.touches[0])
    } else if (e.touches.length === 2) {
      // handleTouchMove () se ocupará del zoom de pellizco.
  ...
  function beforeTouch(e) {
    if (options.onTouch && !options.onTouch(e)) {
        // si devuelven `false` de onTouch, no queremos detenernos
        // propagación de eventos. Correcciones https://github.com/anvaka/panzoom/issues/12
        return
    }

    e.stopPropagation()

    // e.preventDefault() // ** Comente 
  }
