...
var zoom = (function(){
	...
	/**
	 * Desplazar el documento cuando el cursor del mouse se acerca a los bordes.
	* de la ventana.
	 */
	function pan() {
		var range = 0.12,
	...
	return {
		/**
* Hace zoom en un elemento rectángulo o HTML.
*
* Opciones de @param {Objeto}
*
* (requerido)
* - elemento: elemento HTML para acercar
* O
* - x / y: coordenadas en el espacio no transformado para ampliar
* - ancho / alto: la parte de la pantalla para ampliar
* - escala: se puede utilizar en lugar de ancho / alto para establecer explícitamente la escala
*
* (Opcional)
* - devolución de llamada: devuelva la llamada cuando termine de acercarse
* - Relleno: espacio alrededor del elemento ampliado.
*/
		to: function( options ) {

			// Debido a una limitación de implementación no podemos acercar
			// a otro elemento sin alejar primero
			if( level !== 1 ) {
				zoom.out();
			}
			else {
				...
				// Si se establece un elemento, eso tiene prioridad
				if( !!options.element ) {
					// Espacio alrededor del elemento ampliado para dejarlo en la pantalla
